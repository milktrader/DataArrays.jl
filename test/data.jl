module TestData
    ...
    @assert all(array(dvint, replace = 0) .== [1, 2, 0, 4])
    ...
    @assert all(array(pdvstr, replace = "nine") .== ["one", "one", "two", "two", "nine", "one", "one"])
    ...
    @assert isequal(array(dvint, replace = 7), [1, 2, 7, 4])
    @assert sum(removeNA(dvint)) == 7
    @assert sum(array(dvint, replace = 7)) == 14

    #test_group("PooledDataVector Filter and Replace")
    @assert reduce(string, "", removeNA(pdvstr)) == "oneonetwotwooneone"
    @assert reduce(string, "", array(pdvstr, replace = "!")) == "oneonetwotwo!oneone"
