...
# Turn a DataArray into an Array
function array{T}(da::DataArray{T};
                  out::DataType = eltype(da),
                  fail::Bool = true,
                  replace::Any = nothing)
    if replace != nothing
        fail = false
    ...
    if !fail
        if !isa(replace, out)
            msg = @sprintf "Replacement value must be of type %s" out
            error(ArgumentError(msg))
        end
    end
    n = length(da)
    res = Array(out, size(da))
    for i in 1:n
        if da.na[i]
            if fail
                error(NAException())
...
# Turn a DataArray into an Array
function array{T}(da::AbstractDataArray{T};
                  out::DataType = eltype(da),
                  fail::Bool = true,
                  replace::Any = nothing)
    if replace != nothing
